...
export function InviteCustomerDialog({ companies }: InviteCustomerDialogProps) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/admin/invite-customer", {
        method: "POST",
        headers: {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <Card className="relative z-10 w-full max-w-lg rounded-2xl border-neutral-200 shadow-lg ring-0">
            <CardHeader className="border-b border-neutral-200">
              ...
              <CardDescription>
                Send a Supabase invitation email so the customer can set their
                password.
              </CardDescription>
            </CardHeader>
              ...
              <form className="space-y-4" onSubmit={handleSubmit}>
                ...
                {companies.length === 0 && (
                  <p className="text-sm text-neutral-500">
                    Create a company before inviting users.
                  </p>
                )}
